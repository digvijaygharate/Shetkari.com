...
<!-- ********************************************************************************************************************************** -->
 ...
 <h5 class="section-title h1"><span style="color:#1bbd36">Accept</span> <span style="color:black">User Request</span></h5>
     ...
     <form action="edit_wholesaler_status_action.jsp" method="post">
     ...
     <table class="table">
       ...
       try{
			...
			PreparedStatement ps1=con.prepareStatement("select * from applied_ws_user where id=?");
			ps1.setString(1,id);
       ...
       try{
       		Connection con=DbConnection.getConnect();
       		//Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/shetkaricom","root","root");
			PreparedStatement ps1=con.prepareStatement("select * from applied_ws_farmer where id=?");
			ps1.setString(1,id);
			//ps1.setString(2,time);
			ResultSet rs=ps1.executeQuery();
       		while(rs.next())
